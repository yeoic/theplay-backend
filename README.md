...
- **애그리거트 중심 도메인 모델** — 모든 엔티티는 `AggregateRoot`를 상속하며, 도메인 이벤트 등록·발행(`popAllEvents`), 감사 필드(생성/수정/삭제 시각), 소프트 삭제를 공통으로 책임집니다.
- **값 객체(Value Object)** — 주소는 `Address` 임베디드 값 객체로 표현하고 `@EqualsAndHashCode`로 값 동등성을 보장합니다.
- **캡슐화** — 엔티티는 setter를 열지 않고 생성자를 통해서만 만들어지며, 기본 생성자는 `PROTECTED`로 막아 무결성이 깨진 객체 생성을 차단합니다.
