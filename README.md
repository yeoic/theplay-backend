...
- **프로젝트에 종속되지 않는 회사 자원** — 보유 자산(`Asset`)과 작업 공간(`Workspace`)은 특정 프로젝트가 아니라 회사 운영에 속한 자원이므로 별도 애그리거트로 관리합니다. 제 개인적으로는 각 행사에 준비물들이 별도에 있을 것이고 어떤 것들은 직접 준비하시거나 대여하실 것으로 예상했습니다. 귀사 홈페이지의 H-Stage 에 있는 장비 안내 내용을 보고 추측한 내용입니다. 이러한 각 장비들을 '자산' 이라고 명칭했습니다. 대체로 회계 장부에서 자산으로 표현되는 경우가 많은 부분이라서 그렇게 판단하였습니다.
