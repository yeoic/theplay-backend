...
@WebMvcTest(GetAllVenueController.class)
@Import(GetAllVenueDtoMapper.class)
class GetAllVenueControllerTest {

    @Autowired
    MockMvc mockMvc;

    @MockitoBean
    GetAllVenueService getAllVenueService;

    @Test
    @DisplayName("공연장 목록 조회")
    void getAll() throws Exception {
        when(getAllVenueService.getAll(any(GetAllVenueDto.class)))
                .thenReturn(new PageResource<>(List.of(aGetVenueResource()), 0, 20, 1, 1));
